...
def main(argv: List[str]) -> int:
    ap = argparse.ArgumentParser(description="atperf benchmarking results reporting")
    ap.add_argument(
        "--results-dir",
        required=True,
    ...
    # Find downloaded zip artifacts in results_dir (e.g. results_engine_overhead_megabench_cpu_microarchitecture_c7g.metal_20417927879_1.zip)
    ...
    # Build list of payloads with poor/indeterminable data quality
    alerts: List[Dict[str, Any]] = []
    for rs_dict in aggregated_results:
        benchmarking_run_id = rs_dict.get("benchmarking_run_id")
        for p in rs_dict.get("results", []):
            metadata: Dict[str, Any] = p.get("metadata", {})
            dqd = get_dqd(metadata)
            if has_poor_or_indeterminable(dqd):
                benchmark_name = metadata.get("benchmark_name") or "unknown-benchmark"
    ...
    summary_filename = f"engine_results_summary_{ts}"
    summary_path = results_dir / summary_filename
    folder_url, file_url = _resolve_artifactory_urls(summary_filename)

    summary_lines: List[str] = []
    if alerts:
        header_text = "ATP CLI benchmarking alert - benchmarking runs with POOR or INDETERMINABLE accuracy or overhead detected."
        summary_lines.append(header_text)
        summary_lines.append("")
        summary_lines.append(f"Affected benchmarking runs: {len(alerts)}")
        summary_lines.append("")

        lines: List[str] = []
        lines.append(f"*Affected benchmarking runs:* {len(alerts)}")
        lines.append("")
        for idx, a in enumerate(alerts, start=1):
            dqd = a["data_quality_distribution"]
            lines.append(f"*Benchmarking run #{idx} with POOR or INDETERMINABLE accuracy or overhead*")
            lines.append(f"• Benchmark name: {a['benchmark_name']}")
            ...
            lines.append("• RAG status of metrics for accuracy or overhead measured by the benchmark:")
            ...
            lines.append(
                "MODERATE or GOOD RAG indicators meet the ATP release criteria, POOR does not. "
                "INDETERMINABLE indicates missing or invalid data."
            )
            lines.append("")
        detail_text = "\n".join(lines)
    else:
        header_text = "ATP CLI benchmarking report - no POOR or INDETERMINABLE results detected."
        summary_lines.append(header_text)
        summary_lines.append("")
        summary_lines.append("No alerts were detected.")
        summary_lines.append("(Placeholder: there were no benchmark runs with POOR or INDETERMINABLE data quality.)")
        summary_lines.append("")
    ...
    eprintln(f"[INFO] Wrote benchmarking summary: {summary_path}")
    ...
    if not alerts:
        ...
        slack_lines.append(f"Affected benchmarking runs: {len(alerts)}")
        slack_lines.append(f"Summary file: `{summary_filename}`")
        if file_url:
            slack_lines.append(f"Summary of results on Artifactory: <{file_url}|Open summary>")
        elif folder_url:
            slack_lines.append(f"Results folder on Artifactory: <{folder_url}|Open results folder>")
        if run_url:
